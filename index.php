...
if (isset($_GET['acao']) && $_GET['acao'] == 'apagar') {
   ...
   if(mysqli_query($conn,"DELETE FROM lc_movimento WHERE id='$id'"))
   {
    echo mysqli_error();
   }
...
if (isset($_POST['acao']) && $_POST['acao'] == 'editar_cat') {
    ...
    if(mysqli_query($conn,"UPDATE lc_cat SET nome='$nome' WHERE id='$id'"))
    {
        echo mysqli_error();
    }
...
if (isset($_GET['acao']) && $_GET['acao'] == 'apagar_cat') {
    ...
    if(mysqli_query($conn,"DELETE FROM lc_cat WHERE id='$id'"))
    {
        echo mysqli_error();
    }
...
if (isset($_POST['acao']) && $_POST['acao'] == 'editar_mov') {
    ...
    if(mysqli_query($conn,"UPDATE lc_movimento SET dia='$dia', tipo='$tipo', cat='$cat', descricao='$descricao', valor='$valor' WHERE id='$id'"))
    {
        echo mysqli_error();
    }
...
if (isset($_POST['acao']) && $_POST['acao'] == 2) {
    ...
    if(mysqli_query($conn,"INSERT INTO lc_cat (nome) values ('$nome')"))
    {
        echo mysqli_error();
    }
...
if (isset($_POST['acao']) && $_POST['acao'] == 1) {
    ...
    if(mysqli_query($conn,"INSERT INTO lc_movimento (dia,mes,ano,tipo,descricao,valor,cat) values ('$dia','$mes','$ano','$tipo','$descricao','$valor','$cat')"))
    {
        echo mysqli_error();
    }
        ...
        <table cellpadding="1" cellspacing="10" width="900" align="center" style="background-color:#033">
            ...
            <tr>
                ...
                <td colspan="2" align="right" style="background-color:#005B5B;">
                    <a style="color:#FFF"
                       href="?mes=<?php echo date('m') ?>&ano=<?php echo date('Y') ?>">Hoje:<strong> <?php echo date('d') ?>
                            de <?php echo mostraMes(date('m')) ?> de <?php echo date('Y') ?></strong></a>&nbsp;
                </td>
...
<!--                Imprimir os numeros que representa os dias do ano-->
                <td width="70">
...
<!--                Imprimir os nomes dos meses do ano-->
                <?php
                    for ($i = 1; $i <= 12; $i++) {
                        ?>
...
<!--Caixa de movimento e mensagem do sistema-->
        <table cellpadding="10" cellspacing="0" width="900" align="center" border="1">
<!--            Mes Atual/ Botao de Registro-->
            <tr>
                <td colspan="2">
...
<!--Fim da mensagem do sistema-->
                    ...
                    <div style=" background-color:#F1F1F1; padding:10px; border:1px solid #999; margin:5px; display:none"
                         id="add_cat">
                                    ...
                                    while ($row = mysqli_fetch_array($qr)) {
                                        ...
                                        <div id="editar2_cat_<?php echo $row['id'] ?>">
                                            ...
                                            <a style="font-size:10px; color:#666"
                                               onclick="return confirm('Tem certeza que deseja remover esta categoria?\nAtenção: Apenas categorias sem movimentos associados poderão ser removidas.')"
                                               href="?mes=<?php echo $mes_hoje ?>&ano=<?php echo $ano_hoje ?>&acao=apagar_cat&id=<?php echo $row['id'] ?>"
                                               title="Remover">[remover]</a>
                                            <a href="javascript:;" style="font-size:10px; color:#666"
                                               onclick="document.getElementById('editar_cat_<?php echo $row['id'] ?>').style.display=''; document.getElementById('editar2_cat_<?php echo $row['id'] ?>').style.display='none'"
                                        ...
                                        <div style="display:none" id="editar_cat_<?php echo $row['id'] ?>">
                                            ...
                                            <form method="post"
                                                  action="?mes=<?php echo $mes_hoje ?>&ano=<?php echo $ano_hoje ?>">
                                                <input type="hidden" name="acao" value="editar_cat"/>
                                                <input type="hidden" name="id" value="<?php echo $row['id'] ?>"/>
                                                <input type="text" name="nome" value="<?php echo $row['nome'] ?>" size="20"
                                                       maxlength="50"/>
                        ...
                        ?>
                            <form method="post" action="#">
                                ...
                                <input type="text" name="data" size="11" maxlength="10"
                                       value="<?php echo date('d') ?>/<?php echo $mes_hoje ?>/<?php echo $ano_hoje ?>"/>

                                <br/>
                                <br/>

                                <strong>Tipo:</strong>
                                <label for="tipo_receita" style="color:#030"><input type="radio" name="tipo" value="1"
                                                                                    id="tipo_receita"/> Receita</label>&nbsp;
                                <label for="tipo_despesa" style="color:#C00"><input type="radio" name="tipo" value="0"
                                                                                    id="tipo_despesa"/> Despesa</label>
...
<!--Fim Caixa de adicionar movimento-->
                </td>

            </tr>
<!--Fim da Caixa de movimento e mensagem do sistema-->
                    ...
                    <fieldset>
                        ...
                        <table cellpadding="0" cellspacing="0" width="100%">
                            <tr>
                                ...
                                <td align="right"><span
                                            style="font-size:18px; color:#030"><?php echo formata_dinheiro($entradas) ?></span>
                                </td>
                            </tr>
                            <tr>
                                <td><span style="font-size:18px; color:#C00">Saídas:</span></td>
                                <td align="right"><span
                                            style="font-size:18px; color:#C00"><?php echo formata_dinheiro($saidas) ?></span>
                                </td>
                            </tr>
                            <tr>
                                <td colspan="2">
                            ...
                            <tr>
                                <td>
                                    <strong style="font-size:22px; color:<?php if ($resultado_mes < 0) echo "#C00"; else echo "#030" ?>">Resultado:</strong>
                                </td>
                                <td align="right"><strong
                                            style="font-size:22px; color:<?php if ($resultado_mes < 0) echo "#C00"; else echo "#030" ?>"><?php echo formata_dinheiro($resultado_mes) ?></strong>
                                </td>
                            </tr>
                        </table>
                    </fieldset>
                ...
                <td align="left" valign="top" width="450" style="background-color:#F1F1F1">
                    <fieldset>
                        ...
                        <table cellpadding="0" cellspacing="0" width="100%">
                            ...
                            <tr>
                                <td>
                                    <strong style="font-size:22px; color:<?php if ($resultado_geral < 0) echo "#C00"; else echo "#030" ?>">Resultado:</strong>
                                </td>
                                <td align="right"><strong
                                            style="font-size:22px; color:<?php if ($resultado_geral < 0) echo "#C00"; else echo "#030" ?>"><?php echo formata_dinheiro($resultado_geral) ?></strong>
                                </td>
                            </tr>
                        </table>
                ...
                <tr style="display:none; background-color:<?php if ($cont % 2 == 0) echo "#F1F1F1"; else echo "#E0E0E0" ?>"
                    ...
                    <td colspan="3">
                        ...
                        <form method="post" action="?mes=<?php echo $mes_hoje ?>&ano=<?php echo $ano_hoje ?>">
                            ...
                            <b>Tipo:</b> <label for="tipo_receita<?php echo $row['id'] ?>"
                                                style="color:#030"><input <?php if ($row['tipo'] == 1) echo "checked=checked" ?>
                                        type="radio" name="tipo" value="1" id="tipo_receita<?php echo $row['id'] ?>"/>
                                Receita</label>&nbsp; <label for="tipo_despesa<?php echo $row['id'] ?>"
                                                             style="color:#C00"><input <?php if ($row['tipo'] == 0) echo "checked=checked" ?>
                            ...
                            <select name="cat">
                                ...
                                while ($row2 = mysqli_fetch_array($qr2)) {
                                    ?>
                                    <option <?php if ($row2['id'] == $row['cat']) echo "selected" ?>
                                            value="<?php echo $row2['id'] ?>"><?php echo $row2['nome'] ?></option>
                                <?php } ?>
                            </select>&nbsp;|&nbsp;
                            <b>Valor:</b> R$<input type="text" value="<?php echo $row['valor'] ?>" name="valor" size="8"
                                                   maxlength="10"/>
                            <br/>
                            <b>Descricao:</b> <input type="text" name="descricao" value="<?php echo $row['descricao'] ?>"
...
<!--Roda pé-->
        <table cellpadding="5" cellspacing="0" width="900" align="center">
            <tr>
                <td align="right">
                    <hr size="1"/>
                    <em>Livro Caixa - <strong><?php echo $lc_titulo ?></strong> - Melhorado por <a
                                href=https://github.com/marlondantas target="_blank">Marlon Dantas</a>. Versão 2.0 (13/04/2019) <a
                                href="login.php?sair">Fazer logout</a></em>
                </td>
            </tr>
        </table>
